...
shinyServer(function(input, output, session) {
  ...
  build_fit_info_lines <- function(fw) {
    ...
    term_strs[1] <- sprintf("%.4f·1", fit$mu_beta[1])
    for (i in seq_len(p - 1) + 1) {
      term_strs[i] <- sprintf("%.4f·%s", fit$mu_beta[i], x_labels[i])
    }
    linear_eq <- paste0(y_name, " = ", paste(term_strs, collapse = " + "), " + u + \u03b5")
    
    beta_lines <- sprintf("  mu_beta[%d]  %-20s = %+.6f",
                          seq_len(p), paste0("(", x_labels, ")"), fit$mu_beta)
    
    trans_flag <- if (is.null(lambda)) "No" else "Yes"
    trans_lam  <- if (is.null(lambda)) "NA" else sprintf("%.6f", lambda)
    ...
    c(
      ...
      "",
      "[Transformation]",
      sprintf("Yeo-Johnson applied      = %s", trans_flag),
      sprintf("lambda                   = %s", trans_lam),
      "",
      "[Posterior beta | y]",
      beta_lines,
      "",
      ...
      sprintf("eps var.= %.4f,  RMSE = %.4f,  R2 = %.4f",
              fit$eps_var_hat, fit$rmse_total, fit$R2)
    )
  }
